...
export class EditTemplateOperation {
  ...
  public async perform(template: TemplateViewModel): Promise<FailedOrSuccessResult> {
    ...
    var data = await this._dialog.show(TemplateDialog, 
      new TemplateDialogData(
        courses.map(_ => { return { courseId: _.courseId, name: _.name, isSelected: template.courses.some(tc => tc.id == _.courseId) } }), 
        types, template.templateId, template.template, template.templateType, template.courses.map(_ => _.id)));
    if (!data.data) return Result.failed();

    await this._api.send(new SaveTemplateCommand(template.templateId, data.data.template, data.data.templateType, data.data.courseIds)).toPromise();
    return Result.success();
  }
}
