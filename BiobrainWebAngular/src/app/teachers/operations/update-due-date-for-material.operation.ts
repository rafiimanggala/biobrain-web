...
import { UpdateDueDateForLearningMaterialAssignmentCommand } from 'src/app/api/material-assignments/update-due-date-for-learning-material-assignment.command';
...
export class UpdateDueDateForMaterialOperation {
  ...
  public async perform(assignmentId: string, dueDate: Moment): Promise<SuccessOrFailedResult<void>> {
    ...
    try {
      await firstValueFrom(this._api.send(new UpdateDueDateForLearningMaterialAssignmentCommand(assignmentId, dialogResult.data.date, dialogResult.data.date.utc())));
